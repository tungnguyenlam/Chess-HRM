...
def train(
    ...
    # FORCE ACT OFF for supervised training
    ...
    for epoch in range(start_epoch, epochs):
        ...
        samples_seen = 0
        _log(
            f"Epoch {epoch + 1}/{epochs} start | min_elo={current_min_elo} | "
            "waiting for first batch from DataLoader..."
        )
        ...
        for i, (board_t, move_idx, value_t) in enumerate(loader):
            ...
            if pending_batches == accum_steps:
                ...
                pending_data_wait_s = 0.0
                pending_compute_s = 0.0
                should_log = step <= 5 or step % log_every_steps == 0
                if should_log:
                    ...
                    _log(
                        ...
                        f"loss={step_loss:.4f} | "
                        f"avg_loss={avg:.4f} | lr={current_lr:.2e} | norm={grad_norm:.2f}"
                    )
                    running_loss = 0.0
                    running_steps = 0
                    if use_wandb and _WANDB:
        ...
        if epoch_steps > 0:
            ...
            _log(
                f"Epoch {epoch + 1}/{epochs} summary | optimizer_steps=0 | no batches processed"
            )
